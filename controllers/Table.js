...
module.exports.Table = function (io, room) {
  ...
  this.unload = function () {
    // ИСПРАВЛЕНИЕ: Проверяем существование объектов перед очисткой
    if (_oHitAreaShot) {
      if (_oListenerPress) {
        _oHitAreaShot.off("mousedown", _oListenerPress);
      }
      if (_oHitAreaShot.removeAllEventListeners) {
  ...
  this._isValidCueBallPosition = function(oPos) {
    ...
    // Проверка границ стола (реальные границы из FIELD_POINTS)
    // minX: 130 (FIELD_POINTS[1].x), maxX: 1150 (FIELD_POINTS[6].x)
    // minY: 81 (FIELD_POINTS[1].y), maxY: 614 (примерно)
    const TABLE_BOUNDS = {
      minX: 100,
      maxX: 1180,
      minY: 50,
  ...
  this._validateShotForce = function(vStickDirection) {
    if (!vStickDirection || typeof vStickDirection.x !== 'number' || typeof vStickDirection.y !== 'number') {
      console.log("⚠️ Invalid shot data: missing or invalid x/y");
      return false;
    }
    ...
    // MAX_POWER_FORCE_BALL = 40 из setting.js
