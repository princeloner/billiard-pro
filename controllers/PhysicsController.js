...
module.exports.CPhysicsController = function (s_oTable, io) {
  ...
  this._initCollisions = function () {
    ...
    _oRectBall = { x: 0, y: 0, width: 30, height: 30 };
    ...
    oNormal.set(
      (_aFieldEdges[0].getNormal().getX() +
        _aFieldEdges[23].getNormal().getX()) /
        2,
      (_aFieldEdges[0].getNormal().getY() +
        _aFieldEdges[23].getNormal().getY()) /
        2
    );
    oNormal.normalize();
    ...
    for (var i = 0; i < 23; i++) {
      var oTmpNormal = new CVector2(
        (_aFieldEdges[i].getNormal().getX() +
          _aFieldEdges[i + 1].getNormal().getX()) /
          2,
        (_aFieldEdges[i].getNormal().getY() +
          _aFieldEdges[i + 1].getNormal().getY()) /
          2
      );
    ...
    _aPointsNormalsTopLeft.push({
      oPoint: _aPoints[1],
      oNormal: _aPointsNormals[1],
    });
    _aPointsNormalsTopLeft.push({
      oPoint: _aPoints[2],
      oNormal: _aPointsNormals[2],
    });
    _aPointsNormalsTopLeft.push({
      oPoint: _aPoints[5],
      oNormal: _aPointsNormals[5],
    });
    _aPointsNormalsTopLeft.push({
      oPoint: _aPoints[21],
      oNormal: _aPointsNormals[21],
    });

    _aPointsNormalsTopRight.push({
      oPoint: _aPoints[2],
      oNormal: _aPointsNormals[2],
    });
    _aPointsNormalsTopRight.push({
    ...
    _aPointsNormalsTopRight.push({
      oPoint: _aPoints[6],
      oNormal: _aPointsNormals[6],
    });
    _aPointsNormalsTopRight.push({
      oPoint: _aPoints[9],
      oNormal: _aPointsNormals[9],
    });

    _aPointsNormalsBottomRight.push({
      oPoint: _aPoints[10],
      oNormal: _aPointsNormals[10],
    });
    _aPointsNormalsBottomRight.push({
      oPoint: _aPoints[13],
      oNormal: _aPointsNormals[13],
    });
    _aPointsNormalsBottomRight.push({
      oPoint: _aPoints[14],
      oNormal: _aPointsNormals[14],
    });
    _aPointsNormalsBottomRight.push({
      oPoint: _aPoints[17],
      oNormal: _aPointsNormals[17],
    });

    _aPointsNormalsBottomLeft.push({
      oPoint: _aPoints[14],
      oNormal: _aPointsNormals[14],
    });
    _aPointsNormalsBottomLeft.push({
  ...
  this.addBallToStack = function (oBall, oHole) {
    if (oBall.getNumber() === 0) {
      oBall.setPos(CUE_BALL_POS.x, CUE_BALL_POS.y);
  ...
  this.collideBallWithHoles = function (oBall, aHoles) {
    if (distance(oBall.getPos(), aHoles[0]) < BALL_RADIUS) {
      if (_aCbCompleted[ON_BALL_INTO_HOLE]) {
        _aCbCompleted[ON_BALL_INTO_HOLE].call(
          _aCbOwner[ON_BALL_INTO_HOLE],
          oBall
        );
      }
      return aHoles[0];
  ...
  this.collideBallWithPointsNormals = function (oBall, vPos, aPointsNormals) {
    for (var u = 0; u < aPointsNormals.length; u++) {
      if (distance2(aPointsNormals[u].oPoint, vPos) <= BALL_RADIUS_QUADRO) {
        var vReflectedDir = reflectVectorV2(
          oBall.getCurForce(),
          aPointsNormals[u].oNormal
        );
        ...
        if (oBall.getHole() !== null) {
          if (_aCbCompleted[ON_BALL_WITH_BANK]) {
            _aCbCompleted[ON_BALL_WITH_BANK].call(
              _aCbOwner[ON_BALL_WITH_BANK],
              oBall
            );
          }
        }
  ...
  // SPATIAL HASHING: Инициализация сетки на основе размеров стола
  ...
  // SPATIAL HASHING: Построение сетки для текущего кадра
  ...
  // SPATIAL HASHING: Получение близких шаров из соседних ячеек
  ...
  this.collideBallWithBalls = function (oBall) {
    ...
    for (var i = 0; i < nearbyBalls.length; i++) {
      ...
      if (
        otherBall.isBallOnTable() &&
        otherBall.getHole() === null
      ) {
    ...
    if (oBall.getNumber() === 0 && iForceTransfer < 0.3) {
      ...
      iNewForce = this._addBackSpinEffect(
        oBall,
        iValue,
        vRayCollision,
        vDirInvert
    ...
    if (_aCbCompleted[ON_BALL_WITH_BALL]) {
      _aCbCompleted[ON_BALL_WITH_BALL].call(
        _aCbOwner[ON_BALL_WITH_BALL],
        oBall,
        aCollisions[iPos].oBall,
        iForce
      );
    }
  ...
  this._addBackSpinEffect = function (
    oBall,
    iValue,
    vRayCollision,
    vDirInvert
  ...
  this._addSideSpinEffect = function (
    vReflectedDir,
    fRatio,
    oPerpendicularVector,
    oBall
  ) {
    ...
    var iFact = iValue / MAX_SPIN_VALUE;
    iFact = iFact * MAX_SPIN_VALUE;
    var iPreRotation = radiantsToDegrees(
      angleBetweenVectors(vReflectedDir, oPerpendicularVector)
    );
    //        console.log("Pre: " + iPreRotation);
    var vRotateReflection = new CVector2(
      vReflectedDir.getX(),
      vReflectedDir.getY()
    );
    //rotateVector2D(toRadian(iFact* fRatio), vRotateReflection);
    var iPostRotation = radiantsToDegrees(
      angleBetweenVectors(vRotateReflection, oPerpendicularVector)
    );
  ...
  this.checkBallPosInHole = function (oBallInHole, oHole) {
    if (
      oHole === null ||
      oBallInHole.getNumber() === 0 ||
      !oBallInHole.isBallOnTable()
    ) {
      return;
    }
    ...
    if (
      oBallInHole.getCurForceLenght2() < K_MIN_FORCE ||
      distance(oBallInHole.getPos(), oHolePos) > DIST_BALL_HOLE
    ) {
      oBallInHole.setFlagOnTable(false);
      s_oTable._onCheckBallPosInHole(oBallInHole.getNumber());
    }
  };
      ...
      if (oBall.isBallOnTable()) {
        ...
        if (oBall.getHole() === null) {
          ...
          if (oRetHole !== null) {
            ...
            vDirToHole.set(
              oRetHole.getX() - oBall.getX(),
              oRetHole.getY() - oBall.getY()
            );
