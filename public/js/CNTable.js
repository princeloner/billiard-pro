function CNTable(oParentContainer, oCpuDifficultyParams, playerId) {
  ...
  // Ссылка на глобальный socket для доступа из CGame
  this.socket = null;
  var _iPrevState;
  ...
  this._init = function (oCpuDifficultyParams) {
    ...
    // Инициализируем ссылку на глобальный socket
    if (typeof socket !== 'undefined') {
      this.socket = socket;
      console.log("✅ CNTable: Socket connected");
    } else {
      console.log("❌ CNTable: Global socket not found");
    }
  ...
  this.onSocket = function () {
    ...
    // Обработчики таймера - ЗАКОММЕНТИРОВАНО
    /*
  ...
  this.changeTurn = function (data) {
    // Новый формат: data = { fault, currentPlayer, nextTurn }
    // Старый формат: data = bFault (boolean)
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      console.log("🔄 CNTable changeTurn (new format): currentPlayer:", data.currentPlayer, "nextTurn:", nextTurn, "fault:", bFault);

      // Обновляем _currentPlayer
      _currentPlayer = data.currentPlayer === _playerId ? 0 : 1;
    } else {
      ...
      if (_currentPlayer === 0) {
        nextTurn = myNumericId === 1 ? 2 : 1;
      } else {
        nextTurn = myNumericId;
      }

      console.log("🔄 CNTable changeTurn (old format): myId:", _playerId, "nextTurn:", nextTurn, "fault:", bFault);
    }

    s_oGame.netChangeTurn(nextTurn, bFault);
  };
  ...
  this._placeCueBall = function () {
    if (!this._checkCueBallCollisionWithTableElements()) {
      ...
      // Скрываем руку когда биток размещен (если объект создан)
      if (_oHandCueBallDrag) {
        _oHandCueBallDrag.hide();
      }
    }
  ...
  // Обработчики событий таймера от сервера - ЗАКОММЕНТИРОВАНО
  /*
  this.onTimerStart = function(data) {
    console.log("⏱️ Server: Timer started for player:", data.playerId, "Time:", data.timeLeft);

    // Определяем, какой игрок (1 или 2)
    var playerNum = data.playerId === "player1" ? 1 : 2;

    // Запускаем таймер у соответствующего игрока
    if (playerNum === 1 && s_oGame._oPlayer1) {
      s_oGame._oPlayer1.startTimer(data.timeLeft);
    } else if (playerNum === 2 && s_oGame._oPlayer2) {
      s_oGame._oPlayer2.startTimer(data.timeLeft);
    }
  };

  this.onTimerUpdate = function(data) {
    console.log("⏱️ Server: Timer update for player:", data.playerId, "Time left:", data.timeLeft);

    // Определяем, какой игрок (1 или 2)
    var playerNum = data.playerId === "player1" ? 1 : 2;

    // Обновляем таймер у соответствующего игрока
    if (playerNum === 1 && s_oGame._oPlayer1) {
      s_oGame._oPlayer1.updateTimer(data.timeLeft);
    } else if (playerNum === 2 && s_oGame._oPlayer2) {
      s_oGame._oPlayer2.updateTimer(data.timeLeft);
    }
  };

  this.onTimerStop = function() {
    console.log("⏱️ Server: Timer stopped");

    // Останавливаем таймеры у обоих игроков
    if (s_oGame._oPlayer1) {
      s_oGame._oPlayer1.stopTimer();
    }
    if (s_oGame._oPlayer2) {
      s_oGame._oPlayer2.stopTimer();
    }
  };

  this.onTimerTimeout = function(data) {
    console.log("⏰ Server: Timer timeout for player:", data.playerId);

    // Показываем сообщение о таймауте
    new CEffectText("TIME'S UP!", s_oStageUpper3D);

    // Сервер сам отправит changeTurn, просто ждем
  };

  this.onTimerSync = function(data) {
    console.log("🔄 Server: Timer sync for player:", data.playerId, "Time:", data.timeLeft);
