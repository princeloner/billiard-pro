function CTable(oParentContainer, oCpuDifficultyParams) {
  ...
  this._onReleaseHitArea = function () {
    ...
    switch (_iState) {
      case STATE_TABLE_MOVE_STICK: {
        s_oTable._moveStick(
          { x: s_oStage.mouseX, y: s_oStage.mouseY },
          _vMousePosAtHoldStick
        );
    ...
    switch (s_iGameMode) {
      case GAME_MODE_NINE: {
        ...
        if (
          ...
          oBallCollided.getNumber() === _iLowestBall
        ) {
          if (_bCueBallInHole) {
            _bCueBallInHole = false;
            s_oGame.changeTurn();
                ...
                if (s_oGame.isLegalShotFor8Ball(_aBallsInHoleInCurShot[0])) {
                  ...
                  if (_aBallsInHoleInCurShot.length > 1) {
                    for (var i = 1; i < _aBallsInHoleInCurShot.length; i++) {
                      if (
                        !s_oGame.isLegalShotFor8Ball(_aBallsInHoleInCurShot[i])
                      ) {
                        bLegalShot = false;
    ...
    switch (s_iGameMode) {
      case GAME_MODE_NINE: {
        ...
        /*
                              ...
                              if(!bSuccess){*/
        var oHole = this.setHoleToAim(
          _oCueBall,
          _aBalls[_iLowestBall],
          _iLowestBall,
          aEdges
        );
        if (oHole !== null) {
        ...
        if (_aBallsToPotPlayers[s_oGame.getCurTurn() - 1] === 0) {
          ...
          if (oHole !== null) {
            ...
            if (
              ...
              !this.findCollisionForCueBall(
                oHole.coll,
                _aBalls[8],
                new CVector2(oHole.hole.x, oHole.hole.y)
              )
            ) {
              bSuccess = false;
            }
          ...
          for (var t = 0; t < aBalls.length; t++) {
            ...
            if (oHole !== null) {
              ...
              if (DEBUG_SHOW_CPU_BALL_TRAJECTORY) {
                ...
                var oPosBallToCollide = {
                  x: oHole.coll.getX(),
                  y: oHole.coll.getY(),
                };
                _oContainer.addChild(
                  createGraphicCircle(oPosBallToCollide, 6, 10000, "#fff")
                );
              ...
              if (_oCueBall.isDragging()) {
                bSuccess = this.respotCpuCueBall(
                  oHole.coll,
                  aBalls[t].getNumber()
                );
              }
          ...
          if (!bSuccess) {
            ...
            for (var k = iStart; k < iEnd; k++) {
              if (
                (k !== 8 ||
                  _aBallsToPotPlayers[s_oGame.getCurTurn() - 1] === 0) &&
  ...
  this.cpuShot = function (vCueBallDir, fDistance = MAX_FORCE_PER_DISTANCE) {
    ...
    fDistance =
      fDistance > MAX_FORCE_PER_DISTANCE ? MAX_FORCE_PER_DISTANCE : fDistance;
  ...
  this.findCollisionForCueBall = function (vCollPoint, oBallToPot, oHolePoint) {
    if (
      ...
      var fDistance =
        distance(oBallToPot, oHolePoint) +
        distance(oBallToPot, _oCueBall.getPos());
  ...
  s_oTable = this;
  s_oTable._init(oCpuDifficultyParams);
}
